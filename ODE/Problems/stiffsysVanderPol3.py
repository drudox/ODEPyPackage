...
func02 = lambda t,u : 170*(1-u[0]**2)*u[1] - u[0]

func02b = lambda t,u : 20*(1-u[0]**2)*u[1] - u[0]
...
func0b = np.array([func01,func02b])
...
def main():
 ...
 #  problem1 = rhs.Rhs(func0, 0.0, 800 , y0, 60, anal0 , dt=0.003)
   problem2 = rhs.Rhs(func0b, 0.0, 100 , y0, 60, anal0 , dt=0.01)
   x,y = problem2.analiticalSolution(save=True)   
...
#   axs.plot(iet,ieu,label='NR-BWDEuler') #, linestyle=':')
 ...
 #  axs.plot(impt,impu,label='Implicit Mid-Point Method',linestyle='-.')
   ...
   plt.savefig('../Results/VanDerPol3.pdf') 
   plt.show()
