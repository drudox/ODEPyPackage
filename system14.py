...
def main():
    ...
    #############################################       
    # differential problem  Brine Tank Cascade
    #############################################
    eq1 = lambda t,u : -1/6. * u[0] + 1/6.* u[2]      
    eq2 = lambda t,u : 1/6. *u[0] -1/3. *u[1]    
    eq3 = lambda t,u : 1/3.*u[1] - 1/6. *u[2]     
        
    func1 = np.array([eq1,eq2,eq3])
    
    y0      = np.array([10,90.,25.])
    system1 = rhs.Rhs(func1, 0,15,y0,40 )
...
#-----------------------------------------------------------------------------------------------------   
    ...
    axs.set_title('Brine Tank Cascade II',y=1.05)
    plt.savefig('ODE/Results/system14.pdf')   
    plt.show()
