...
def main():
    ...
    #############################################
    eq1 = lambda t,u : -0.5 * u[0]       
    eq2 = lambda t,u : 0.5*u[0] - 0.25 *u[1]     
    eq3 = lambda t,u : 0.25*u[1] - 1/6. *u[2]     
        
    func1 = np.array([eq1,eq2,eq3])
    
    y0      = np.array([20.,40.,60.])
    system1 = rhs.Rhs(func1, 0,40,y0,50 )
...
#-----------------------------------------------------------------------------------------------------   
    ...
    axs.set_title('Brine Tank Cascade',y=1.05)
    plt.savefig('ODE/Results/system13.pdf')   
    plt.show()
